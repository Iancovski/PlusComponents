...
type
  ...
  private
    ...
    FIsFontScaled: Boolean;
    FCheckBox: Boolean;
    FCheckBoxValues: TCheckBoxValues;
    procedure FontChanged(Sender: TObject);
    function  GetAlignment: TAlignment;
    function  GetColor: TColor;
    ...
    procedure SetCheckBoxValues(const Value: TCheckBoxValues);
  protected
    function  CreateTitle: TColumnTitle; virtual;
    function  GetGrid: TCustomPDBGrid;
  ...
  public
    ...
    procedure RestoreDefaults; virtual;
    property  Grid: TCustomPDBGrid read GetGrid;
  ...
  public
    constructor Create(Grid: TCustomPDBGrid; ColumnClass: TColumnClass);
    ...
    property Grid: TCustomPDBGrid read FGrid;
  ...
  public
    constructor Create(AGrid: TCustomPDBGrid);
    destructor Destroy; override;
  ...
  TCustomPDBGrid = class(TCustomGrid)
  private
...
begin
  ...
  begin
    ...
    if Accept then
      if ActiveList = DataList then
        with TCustomPDBGrid(Grid), Columns[SelectedIndex].Field do
        begin
...
begin
  TCustomPDBGrid(Grid).EditButtonClick;
end;
  ...
  begin
    ...
    if ActiveList = FDataList then
      ...
      begin
        ...
        FDataList.Font := Font;
        FDataList.StyleElements := TCustomPDBGrid(Grid).StyleElements;
        FDataList.StyleName := TCustomPDBGrid(Grid).StyleName;
...
constructor TGridDataLink.Create(AGrid: TCustomPDBGrid);
begin
  inherited Create;
  FGrid := AGrid;
  VisualControl := True;
...
function TColumn.GetGrid: TCustomPDBGrid;
begin
  if Assigned(Collection) and (Collection is TDBGridColumns) then
...
procedure TColumn.SetCheckBox(const Value: Boolean);
begin
  FCheckBox := Value;
end;

procedure TColumn.SetCheckBoxValues(const Value: TCheckBoxValues);
begin
...
constructor TDBGridColumns.Create(Grid: TCustomPDBGrid; ColumnClass: TColumnClass);
begin
...
constructor TBookmarkList.Create(AGrid: TCustomPDBGrid);
begin
  inherited Create;
  SetLength(FList, 0);
  FGrid := AGrid;
...
class constructor TCustomPDBGrid.Create;

  procedure InitIndicatorImageCollection;
...
begin
  TCustomStyleEngine.RegisterStyleHook(TCustomPDBGrid, TScrollingStyleHook);
  InitIndicatorImageCollection;
end;

constructor TCustomPDBGrid.Create(AOwner: TComponent);
begin
...
class destructor TCustomPDBGrid.Destroy;
begin
  FreeAndNil(FIndicatorImageCollection);
  TCustomStyleEngine.UnRegisterStyleHook(TCustomPDBGrid, TScrollingStyleHook);
end;

destructor TCustomPDBGrid.Destroy;
begin
  FColumns.Free;
  FColumns := nil;
...
function TCustomPDBGrid.AcquireFocus: Boolean;
begin
  Result := True;
...
procedure TCustomPDBGrid.ChangeScale(M, D: Integer; isDpiChange: Boolean);
var
  I: Integer;
  Save: TNotifyEvent;
begin
...
function TCustomPDBGrid.RawToDataColumn(ACol: Integer): Integer;
begin
  Result := ACol - FIndicatorOffset;
end;

function TCustomPDBGrid.DataToRawColumn(ACol: Integer): Integer;
begin
  Result := ACol + FIndicatorOffset;
end;

function TCustomPDBGrid.AcquireLayoutLock: Boolean;
begin
  Result := (FUpdateLock = 0) and (FLayoutLock = 0);
  if Result then BeginLayout;
end;

procedure TCustomPDBGrid.BeginLayout;
begin
  BeginUpdate;
...
procedure TCustomPDBGrid.BeginUpdate;
begin
  Inc(FUpdateLock);
end;

procedure TCustomPDBGrid.CancelLayout;
begin
...
function TCustomPDBGrid.CanEditAcceptKey(Key: Char): Boolean;
begin
  with Columns[SelectedIndex] do
    Result := FDatalink.Active and Assigned(Field) and Field.IsValidChar(Key);
end;

function TCustomPDBGrid.CanEditModifyField(AField: TField): Boolean;
begin
  Result := False;
...
function TCustomPDBGrid.CanEditModify: Boolean;
begin
  Result := CanEditModifyField(Columns[SelectedIndex].Field);
end;

function TCustomPDBGrid.CanEditShow: Boolean;
begin
  Result := (LayoutLock = 0) and inherited CanEditShow;
end;

procedure TCustomPDBGrid.CellClick(Column: TColumn);
begin
  if Assigned(FOnCellClick) then FOnCellClick(Column);
end;

procedure TCustomPDBGrid.ColEnter;
begin
  UpdateIme;
  if Assigned(FOnColEnter) then FOnColEnter(Self);
end;

procedure TCustomPDBGrid.ColExit;
begin
  if Assigned(FOnColExit) then FOnColExit(Self);
end;

procedure TCustomPDBGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
...
procedure TCustomPDBGrid.ColWidthsChanged;
var
  I: Integer;
begin
...
function TCustomPDBGrid.CreateColumns: TDBGridColumns;
begin
  Result := TDBGridColumns.Create(Self, TColumn);
end;

function TCustomPDBGrid.CreateDataLink: TGridDataLink;
begin
  Result := TGridDataLink.Create(Self);
end;

function TCustomPDBGrid.CreateEditor: TInplaceEdit;
begin
  Result := TDBGridInplaceEdit.Create(Self);
end;

procedure TCustomPDBGrid.CreateWnd;
begin
...
procedure TCustomPDBGrid.DataChanged;
begin
...
procedure TCustomPDBGrid.DefaultHandler(var Msg);
var
  P: TPopupMenu;
  Cell: TGridCoord;
begin
...
procedure TCustomPDBGrid.DeferLayout;
var
  M: TMsg;
begin
...
procedure TCustomPDBGrid.DefineFieldMap;
var
  I: Integer;
begin
...
function TCustomPDBGrid.UseRightToLeftAlignmentForField(const AField: TField;
  Alignment: TAlignment): Boolean;
begin
  Result := False;
...
procedure TCustomPDBGrid.DefaultDrawDataCell(const Rect: TRect; Field: TField;
...
procedure TCustomPDBGrid.DefaultDrawColumnCell(const Rect: TRect;
...
procedure TCustomPDBGrid.ReadColumns(Reader: TReader);
begin
  Columns.Clear;
  Reader.ReadValue;
...
procedure TCustomPDBGrid.WriteColumns(Writer: TWriter);
begin
...
procedure TCustomPDBGrid.DefineProperties(Filer: TFiler);
var
  StoreIt: Boolean;
...
begin
  ...
  if Filer.Ancestor = nil then
    ...
    if vState <> TCustomPDBGrid(Filer.Ancestor).Columns.State then
      StoreIt := True
    else
      StoreIt := (vState = csCustomized) and
        (not CollectionsEqual(Columns, TCustomPDBGrid(Filer.Ancestor).Columns, Self, TCustomPDBGrid(Filer.Ancestor)));

  Filer.DefineProperty('Columns', ReadColumns, WriteColumns, StoreIt);
end;

function TCustomPDBGrid.ColumnAtDepth(Col: TColumn; ADepth: Integer): TColumn;
begin
  Result := Col;
...
function TCustomPDBGrid.CalcTitleRect(Col: TColumn; ARow: Integer;
  var MasterCol: TColumn): TRect;
var
  I,J: Integer;
  InBiDiMode: Boolean;
...
function TCustomPDBGrid.CalcExpandedCellRect(const Coord: TGridCoord): TGridRect;
var
  MasterCol: TColumn;
  ScreenRect: TRect;
begin
...
function TCustomPDBGrid.CalcColWidth(const ATextLength: Integer; const ACaption: string;
...
procedure TCustomPDBGrid.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var
  FrameOffs: Byte;
...
procedure TCustomPDBGrid.DrawDataCell(const Rect: TRect; Field: TField;
  State: TGridDrawState);
begin
  if Assigned(FOnDrawDataCell) then FOnDrawDataCell(Self, Rect, Field, State);
end;

procedure TCustomPDBGrid.DrawCellBackground(const ARect: TRect; AColor: TColor;
...
procedure TCustomPDBGrid.DrawCellHighlight(const ARect: TRect;
  AState: TGridDrawState; ACol, ARow: Integer);
begin
...
procedure TCustomPDBGrid.DrawColumnCell(const Rect: TRect; DataCol: Integer;
  Column: TColumn; State: TGridDrawState);
begin
...
procedure TCustomPDBGrid.EditButtonClick;
begin
...
procedure TCustomPDBGrid.EditingChanged;
begin
  if dgIndicator in Options then InvalidateCell(0, FSelRow);
end;

procedure TCustomPDBGrid.EndLayout;
begin
  if FLayoutLock > 0 then
  begin
    try
...
procedure TCustomPDBGrid.EndUpdate;
begin
  if FUpdateLock > 0 then
    Dec(FUpdateLock);
end;

function TCustomPDBGrid.GetColField(DataCol: Integer): TField;
begin
  Result := nil;
...
function TCustomPDBGrid.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TCustomPDBGrid.GetEditLimit: Integer;
begin
  Result := 0;
...
function TCustomPDBGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
...
function TCustomPDBGrid.GetEditStyle(ACol, ARow: Integer): TEditStyle;
var
  Column: TColumn;
  MasterField: TField;
begin
...
function TCustomPDBGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
  if FDatalink.Active then
    with Columns[RawToDataColumn(ACol)] do
...
function TCustomPDBGrid.GetFieldCount: Integer;
begin
  Result := FDatalink.FieldCount;
end;

function TCustomPDBGrid.GetFields(FieldIndex: Integer): TField;
begin
  Result := FDatalink.Fields[FieldIndex];
end;

function TCustomPDBGrid.GetFieldValue(ACol: Integer): string;
var
  Field: TField;
begin
  Result := '';
...
function TCustomPDBGrid.GetSelectedField: TField;
var
  Index: Integer;
begin
  Index := SelectedIndex;
...
function TCustomPDBGrid.GetSelectedIndex: Integer;
begin
  Result := RawToDataColumn(Col);
end;

function TCustomPDBGrid.HighlightCell(DataCol, DataRow: Integer;
...
procedure TCustomPDBGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TCustomPDBGrid.KeyPress(var Key: Char);
begin
  FIsESCKey := False;
...
procedure TCustomPDBGrid.InternalLayout;
...
procedure TCustomPDBGrid.LayoutChanged;
begin
  if AcquireLayoutLock then
    EndLayout;
end;

procedure TCustomPDBGrid.LinkActive(Value: Boolean);
var
  Comp: TComponent;
  I: Integer;
begin
  ...
  try
    ...
    begin
      ...
      if (Comp is TCustomPDBGrid)
        and (TCustomPDBGrid(Comp).DragKind = dkDock) then
        Comp.Free;
    end;
    UpdateScrollBar;
...
procedure TCustomPDBGrid.Loaded;
begin
  inherited Loaded;
...
function TCustomPDBGrid.PtInExpandButton(X,Y: Integer; var MasterCol: TColumn): Boolean;
var
  Cell: TGridCoord;
  R: TRect;
begin
...
procedure TCustomPDBGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
...
procedure TCustomPDBGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
...
function TCustomPDBGrid.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := False;
...
function TCustomPDBGrid.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := False;
...
procedure TCustomPDBGrid.MoveCol(RawCol, Direction: Integer);
var
  OldCol: Integer;
begin
  FDatalink.UpdateData;
...
procedure TCustomPDBGrid.Notification(AComponent: TComponent;
...
procedure TCustomPDBGrid.RecordChanged(Field: TField);
var
  I: Integer;
  CField: TField;
begin
...
procedure TCustomPDBGrid.Scroll(Distance: Integer);
var
...
procedure TCustomPDBGrid.SetColumns(Value: TDBGridColumns);
begin
...
procedure TCustomPDBGrid.SetColumnAttributes;
var
  I: Integer;
begin
  for I := 0 to FColumns.Count-1 do
...
procedure TCustomPDBGrid.SetDataSource(Value: TDataSource);
begin
...
procedure TCustomPDBGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  FEditText := Value;
end;

procedure TCustomPDBGrid.SetOptions(Value: TDBGridOptions);
const
...
procedure TCustomPDBGrid.SetSelectedField(Value: TField);
var
  I: Integer;
begin
...
procedure TCustomPDBGrid.SetSelectedIndex(Value: Integer);
begin
  MoveCol(DataToRawColumn(Value), 0);
end;

procedure TCustomPDBGrid.SetTitleFont(Value: TFont);
begin
  FTitleFont.Assign(Value);
  if dgTitles in Options then LayoutChanged;
end;

function TCustomPDBGrid.StoreColumns: Boolean;
begin
  Result := Columns.State = csCustomized;
end;

procedure TCustomPDBGrid.TimedScroll(Direction: TGridScrollDirection);
begin
  if FDatalink.Active then
  begin
    with FDatalink do
...
procedure TCustomPDBGrid.TitleClick(Column: TColumn);
begin
  if (dgTitleClick in FOptions) and Assigned(FOnTitleClick) then
    FOnTitleClick(Column);
end;

procedure TCustomPDBGrid.TitleFontChanged(Sender: TObject);
begin
...
procedure TCustomPDBGrid.UpdateActive;
var
  NewRow: Integer;
  Field: TField;
begin
...
procedure TCustomPDBGrid.UpdateData;
var
  Field: TField;
begin
  Field := SelectedField;
...
procedure TCustomPDBGrid.UpdateRowCount;
var
  OldRowCount: Integer;
begin
  ...
  with FDataLink do
    ...
    else if not FDataLink.DataSet.ControlsDisabled then
    begin
      RowCount := 1000;
...
procedure TCustomPDBGrid.UpdateScrollBar;
var
...
function TCustomPDBGrid.ValidFieldIndex(FieldIndex: Integer): Boolean;
begin
  Result := DataLink.GetMappedIndex(FieldIndex) >= 0;
end;

procedure TCustomPDBGrid.CMParentFontChanged(var Message: TMessage);
begin
  inherited;
  if ParentFont then
  begin
...
procedure TCustomPDBGrid.CMBiDiModeChanged(var Message: TMessage);
var
  Loop: Integer;
begin
  inherited;
  for Loop := 0 to ComponentCount - 1 do
    if Components[Loop] is TCustomPDBGrid then
      with Components[Loop] as TCustomPDBGrid do
        { Changing the window, echos down to the subgrid }
        if Parent <> nil then
          Parent.BiDiMode := Self.BiDiMode;
end;

procedure TCustomPDBGrid.CMExit(var Message: TMessage);
begin
  try
    if FDatalink.Active then
      with FDatalink.Dataset do
...
procedure TCustomPDBGrid.CMFontChanged(var Message: TMessage);
var
  I: Integer;
begin
  inherited;
...
procedure TCustomPDBGrid.CMDeferLayout(var Message);
begin
  if AcquireLayoutLock then
    EndLayout
  else
...
procedure TCustomPDBGrid.CMDesignHitTest(var Msg: TCMDesignHitTest);
var
  MasterCol: TColumn;
begin
  inherited;
...
procedure TCustomPDBGrid.WMSetCursor(var Msg: TWMSetCursor);
begin
...
procedure TCustomPDBGrid.WMSize(var Message: TWMSize);
begin
  inherited;
...
procedure TCustomPDBGrid.WMVScroll(var Message: TWMVScroll);
var
  SI: TScrollInfo;
begin
  if not AcquireFocus then Exit;
...
procedure TCustomPDBGrid.SetIme;
var
  Column: TColumn;
begin
...
procedure TCustomPDBGrid.UpdateIme;
begin
  SetIme;
  SetImeName(ImeName);
...
procedure TCustomPDBGrid.WMIMEStartComp(var Message: TMessage);
begin
  inherited;
  ShowEditor;
end;

procedure TCustomPDBGrid.WMSetFocus(var Message: TWMSetFocus);
begin
  FNeedRestoreImeName := False;
...
procedure TCustomPDBGrid.WMKillFocus(var Message: TMessage);
begin
...
function TCustomPDBGrid.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := (DataLink <> nil) and DataLink.ExecuteAction(Action);
end;

function TCustomPDBGrid.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := (DataLink <> nil) and DataLink.UpdateAction(Action);
end;

procedure TCustomPDBGrid.ShowPopupEditor(Column: TColumn; X, Y: Integer);
var
  SubGrid: TCustomPDBGrid;
  DS: TDataSource;
  I: Integer;
  FloatRect: TRect;
  Cmp: TControl;
begin
  ...
  for I := 0 to ComponentCount-1 do
    if Components[I] is TCustomPDBGrid then
    begin
      SubGrid := TCustomPDBGrid(Components[I]);
  ...
  SubGrid := TCustomPDBGrid(TComponentClass(Self.ClassType).Create(Self));
  try
...
//    SubGrid.ManualDock(nil,nil,alClient);
    ...
    Cmp := Self;
    while (Cmp <> nil) and (TCustomPDBGrid(Cmp).PopupMenu = nil) do
      Cmp := Cmp.Parent;
    if Cmp <> nil then
      SubGrid.PopupMenu := TCustomPDBGrid(Cmp).PopupMenu;
...
procedure TCustomPDBGrid.CalcSizingState(X, Y: Integer;
...
function TCustomPDBGrid.CheckColumnDrag(var Origin, Destination: Integer;
...
function TCustomPDBGrid.BeginColumnDrag(var Origin, Destination: Integer;
  const MousePt: TPoint): Boolean;
var
  I, ARow: Integer;
begin
...
function TCustomPDBGrid.EndColumnDrag(var Origin, Destination: Integer;
  const MousePt: TPoint): Boolean;
begin
...
procedure TCustomPDBGrid.InvalidateTitles;
var
  R: TRect;
  DrawInfo: TGridDrawInfo;
begin
...
procedure TCustomPDBGrid.TopLeftChanged;
begin
  InvalidateTitles;
